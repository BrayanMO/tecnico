<div class=" bg-white py-6 flex flex-col justify-center sm:py-12">
    <div class="relative py-3 w-11/12 max-w-xl sm:mx-auto">
        <div class="relative p-8 bg-white shadow-sm sm:rounded-xl">
            <form class="w-full" action="{{ route('tipoequipo.update',$tipoequipo) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-5 relative">
                    <input type="text" id="text"
                        ...
                        name="description" value="{{ old('description', $tipoequipo->description) }}"/>
                        @error('description')
                        {{-- Mensaje de error --}}
                ...
                <button type="submit" class="w-full bg-indigo-600 text-white p-3 rounded-md">Editar Equipo</button>
            </form>
        </div>
    </div>
</div>